...
const TIP_DURATION = 4_000;

const STAGE_AT = [0, 5_000, 10_500];

const STAGE_LABELS = [
  { label: 'Building your collection', sub: 'Grouping your design products into a sendable collection' },
  { label: 'Applying your brand',      sub: 'Your logo will appear on every item recipients see' },
  { label: 'Collection ready',         sub: 'Set a budget — each recipient sees only items they can afford' },
];

const TIPS = [
  { label: 'How collections work', headline: 'Each recipient sees their own view', body: 'You set a max budget — recipients only see items they can afford. No awkward over/under.' },
  { label: 'Smart branding',       headline: 'Logo placed perfectly, every time', body: 'Your logo is applied across all product types at optimal placement.' },
  { label: 'Budget-aware',         headline: 'One collection, many budgets', body: 'Send the same collection to your whole team. Everyone sees what fits their allocation.' },
  { label: 'Global reach',         headline: 'Adapts to each country automatically', body: 'Products and availability adjust per recipient\'s region. No manual filtering.' },
];
...
export function CollectionBuildingLoader({ collectionName, productIds, logoUrl, onComplete }: Props) {
  ...
  useEffect(() => {
    ...
    // Tip rotation
    const tipTimers: ReturnType<typeof setTimeout>[] = [];
    TIPS.forEach((_, i) => {
      if (i === 0) return;
      tipTimers.push(
        setTimeout(() => {
          setTipVisible(false);
          setTimeout(() => { setTipIndex(i % TIPS.length); setTipVisible(true); }, 250);
        }, i * TIP_DURATION),
      );
    });
    ...
    return () => {
      ...
      [s1, s2, end].forEach(clearTimeout);
  ...
  return (
    <>
      <style>{`
        @keyframes cbl-orbit     { to { transform: rotate(360deg); } }
        @keyframes cbl-orbitRev  { to { transform: rotate(-360deg); } }
        @keyframes cbl-pulse {
      ...
      <div style={{
        ...
        <div style={{
          ...
          animation: 'cbl-pulse 5s ease-in-out infinite',
          pointerEvents: 'none',
        }} />
        <div style={{
          ...
          top: '12%', right: '8%',
          animation: 'cbl-pulse 7s ease-in-out 1.5s infinite',
          pointerEvents: 'none',
        }} />
        <div style={{
          ...
          bottom: '10%', left: '6%',
          animation: 'cbl-pulse 6s ease-in-out 3s infinite',
          pointerEvents: 'none',
        }} />
        ...
        {SPARKLES.map((s, i) => (
          <div key={i} style={{
            ...
            animation: `cbl-twinkle ${s.dur} ease-in-out ${s.delay} infinite`,
            pointerEvents: 'none',
          }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '0 24px' }}>
          ...
          <div style={{ position: 'relative', display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: 22 }}>
            <div style={{
              ...
              animation: 'cbl-orbit 2.5s linear infinite',
            }} />
            <div style={{
              ...
              animation: 'cbl-orbitRev 4.2s linear infinite',
            }} />
            <div style={{
              ...
              overflow: 'hidden',
              animation: stage === 0 ? 'cbl-glow 2.2s ease-in-out infinite' : undefined,
            }}>
              {logoUrl ? (
              ...
              {stage === 0 && (
                <div style={{
                  ...
                  animation: 'cbl-scan 2s ease-in-out infinite',
                }} />
              )}
              ...
              {completing && (
                <div style={{
                  ...
                  display: 'flex', alignItems: 'center', justifyContent: 'center',
                  animation: 'cbl-checkGrow 0.5s cubic-bezier(0.34,1.56,0.64,1) forwards',
                }}>
                  <svg width="36" height="36" viewBox="0 0 36 36" fill="none">
                    ...
                    <path d="M10 18L16 24L26 13" stroke="#22c55e" strokeWidth="2.2"
                      ...
                      strokeDasharray="24"
                      style={{ animation: 'cbl-tickDrawLg 0.4s ease 0.2s forwards', strokeDashoffset: 24 }} />
                  </svg>
                </div>
              )}
            </div>
          ...
          <div style={{ textAlign: 'center', marginBottom: 28 }}>
            <h1
              ...
              style={{
                ...
                animation: 'cbl-fadeUp 0.45s ease forwards',
              }}
            >
              {completing ? 'Collection ready!' : collectionName}
            </h1>
            <p
              key={`sub-${stage}`}
              style={{
                fontSize: 14, color: 'rgba(255,255,255,0.45)', margin: 0,
                animation: 'cbl-fadeUp 0.4s ease 0.05s forwards', opacity: 0,
              }}
            >
              ...
              return (
                <div
                  ...
                  style={{
                    ...
                    animation: visible
                      ? `cbl-slideUp 0.5s cubic-bezier(0.34,1.2,0.64,1) forwards, cbl-float ${3.2 + i * 0.4}s ease-in-out ${i * 0.35}s infinite`
                      : 'none',
                  }}
                >
                  {/* Image */}
                  ...
                  {hasBadge && !hasTick && logoUrl && (
                    <div style={{
                      ...
                      display: 'flex', alignItems: 'center', justifyContent: 'center',
                      overflow: 'hidden',
                      animation: 'cbl-badgePop 0.5s cubic-bezier(0.34,1.56,0.64,1) forwards, cbl-badgeRing 1.6s ease-in-out infinite',
                    }}>
                  ...
                  {hasTick && (
                    <div style={{
                      ...
                      display: 'flex', alignItems: 'center', justifyContent: 'center',
                      animation: 'cbl-checkGrow 0.4s cubic-bezier(0.34,1.56,0.64,1) forwards',
                    }}>
                      <svg width="13" height="13" viewBox="0 0 13 13" fill="none">
                        <path d="M2.5 6.5L5.5 9.5L10.5 3.5" stroke="white" strokeWidth="1.9"
                          strokeLinecap="round" strokeLinejoin="round"
                          strokeDasharray="14"
                          style={{ animation: 'cbl-tickDraw 0.3s ease 0.1s forwards', strokeDashoffset: 14 }} />
                      </svg>
                    </div>
                  )}
                </div>
