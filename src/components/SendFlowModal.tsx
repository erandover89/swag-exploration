...
function CampaignCard({ c }: { c: Campaign }) {
  return (
    <div className="group flex items-center gap-3 px-3 py-2.5 rounded-[12px] bg-[#f8fafc] hover:bg-snp-indigo-50 border border-transparent hover:border-[#c7d7f4] transition-all cursor-pointer">
      <div className="w-8 h-8 rounded-[9px] bg-white border border-snp-navy-200 flex items-center justify-center text-base shrink-0 shadow-sm">
        {c.emoji}
      </div>
      <div className="flex-1 min-w-0">
        <p className="text-[13px] font-semibold text-snp-navy-950 truncate leading-tight">{c.name}</p>
        <p className="text-[11px] text-snp-navy-500 mt-0.5">{c.sent} recipients · ${c.spent.toLocaleString()} · {c.date}</p>
      </div>
      <button
        className="shrink-0 flex items-center gap-1 text-[11px] font-semibold text-snp-indigo-600 opacity-0 group-hover:opacity-100 transition-opacity bg-white border border-[#c7d7f4] rounded-[7px] px-2.5 py-1 hover:bg-snp-indigo-50"
...
function CollectionCard({ col, onClick }: { col: GiftCollection; onClick: () => void }) {
  ...
  return (
    <button
      onClick={onClick}
      className="group flex flex-col text-left rounded-[20px] overflow-hidden bg-white border border-snp-navy-200 hover:border-snp-indigo-600 transition-all duration-200 hover:shadow-[0px_12px_32px_rgba(48,119,201,0.16)] hover:-translate-y-1"
      style={{ fontFamily: "'DM Sans', sans-serif" }}
    >
      {/* 2×2 collage */}
      <div className="grid grid-cols-2 gap-1 p-2 bg-snp-navy-50">
        {products.slice(0, 4).map(p => (
          <div key={p.id} className="aspect-square rounded-[8px] overflow-hidden bg-white flex items-center justify-center">
            {p.image.startsWith('/') ? (
              <img src={p.image} alt={p.name} className="w-full h-full object-contain" style={{ mixBlendMode: 'multiply' }} />
            ) : (
      ...
      <div className="px-3 pt-2 pb-3">
        <p className="text-[13px] font-bold text-snp-navy-950 leading-snug">{col.name}</p>
        <p className="text-[11px] text-snp-navy-500 mt-0.5 leading-tight">{col.sub}</p>
      </div>
    </button>
  );
}
...
export function SendFlowModal({ onClose }: { onClose: () => void }) {
  ...
  const handleCollectionClick = (col: GiftCollection) => {
    if (col.ai) { alert('Coming soon!'); return; }
      ...
      <div className="sf-root min-h-screen flex flex-col">
        ...
        <div className="sticky top-0 z-10 bg-white/90 backdrop-blur-md border-b border-[#f0f4f8] flex items-center justify-between px-6 md:px-14 py-4">
          <div className="flex items-center gap-2.5">
            ...
            <span className="text-[12px] font-bold text-snp-navy-400 tracking-[0.14em] uppercase">Send a Gift</span>
          </div>
          <button
            onClick={onClose}
            className="w-9 h-9 rounded-full bg-snp-navy-50 hover:bg-snp-navy-200 flex items-center justify-center text-snp-navy-500 hover:text-snp-navy-700 transition-colors"
          >
            <X className="w-4 h-4" />
          </button>
        </div>
        ...
        <div className="flex-1 max-w-[860px] mx-auto w-full px-6 md:px-8 pt-6 pb-8">
          ...
          {/* ── Hero ─────────────────────────────────────────────────── */}
          <div className="mb-5">
            <p className="text-[10px] font-bold text-snp-indigo-600 uppercase tracking-[0.15em] mb-1.5">Let's send something great</p>
            <h1
              ...
              style={{
                ...
                fontWeight: 700,
                color: 'var(--snp-navy-950)',
              }}
            >
              Send something{' '}
              <span style={{
                backgroundImage: 'linear-gradient(88deg, var(--snp-indigo-600) 0%, #36d4ff 100%)',
            ...
            <section className="mb-5">
              <div className="flex items-center justify-between mb-2">
                <div className="flex items-center gap-1.5">
                  <RotateCcw className="w-3 h-3 text-snp-navy-500" />
                  <span className="text-[10px] font-bold text-snp-navy-500 uppercase tracking-widest">Sent before</span>
                </div>
                <div className="flex items-center gap-3">
                  <button className="text-[12px] font-semibold text-snp-indigo-600 hover:underline">
                    See All →
                  </button>
                  <button
                    onClick={() => setShowSendAgain(false)}
                    title="Dismiss"
                    className="w-5 h-5 rounded-full bg-snp-navy-50 hover:bg-snp-navy-200 flex items-center justify-center text-snp-navy-500 hover:text-snp-navy-700 transition-colors"
                  >
                    <X className="w-3 h-3" />
                  </button>
                </div>
          ...
          {/* ── Budget + Country ─────────────────────────────────────── */}
          <section className="mb-5">
            <p className="text-[10px] font-bold text-snp-navy-500 uppercase tracking-widest mb-3">
            ...
            >
              <span className="text-snp-navy-950" style={{ fontSize: 'clamp(18px, 3vw, 26px)', fontWeight: 700 }}>
                My budget is
              </span>
              ...
              <div className="relative flex items-center">
                <span
                  className="absolute left-3 text-snp-navy-500 pointer-events-none select-none"
                ...
                <input
                  ...
                  className="pl-9 pr-3 h-[44px] w-[110px] border-2 rounded-[12px] focus:outline-none focus:ring-2 focus:ring-snp-indigo-600/10 transition-all bg-[#f9fbff] text-snp-navy-950"
                  style={{
                    borderColor: budgetValid ? 'var(--snp-indigo-600)' : 'var(--snp-navy-200)',
              ...
              <span className="text-snp-navy-500" style={{ fontSize: 'clamp(16px, 2.5vw, 22px)', fontWeight: 600 }}>
              ...
              <div className="relative" ref={countryRef}>
                <button
                  ...
                  className="flex items-center gap-2 h-[44px] px-3 border-2 rounded-[12px] transition-all bg-[#f9fbff] text-snp-navy-950 font-semibold"
                  style={{
                    borderColor: selectedCountries.length > 0 ? 'var(--snp-indigo-600)' : 'var(--snp-navy-200)',
                    fontFamily: "'DM Sans', sans-serif",
                    fontSize: '14px',
                  }}
                >
                  <Globe2 className="w-4 h-4 text-snp-navy-500 shrink-0" />
                  <span className="max-w-[180px] truncate">{countryLabel()}</span>
                  <ChevronDown className={`w-4 h-4 text-snp-navy-500 shrink-0 transition-transform duration-200 ${showCountryMenu ? 'rotate-180' : ''}`} />
                </button>
                {showCountryMenu && (
                  <div className="absolute top-full mt-2 left-0 bg-white border border-snp-navy-200 rounded-[16px] shadow-[0px_12px_40px_rgba(1,39,84,0.14)] z-20 min-w-[220px] py-2 overflow-hidden">
                    {COUNTRIES.map(c => {
                      ...
                      return (
                        <button
                          ...
                          className={`w-full flex items-center gap-3 px-4 py-2.5 text-[14px] font-medium text-left transition-colors ${sel ? 'bg-snp-indigo-50 text-snp-indigo-600' : 'text-snp-navy-700 hover:bg-snp-navy-50'}`}
                        >
            ...
            {/* Inline hint */}
            {budget && !budgetValid && (
              <p className="mt-3 text-[12px] text-snp-navy-500">Enter a budget of at least $5 to see recommendations.</p>
            )}
          </section>
          ...
          {collectionsVisible && (
            <section className="sf-collections mb-5">
              <div className="flex items-center gap-3 mb-3">
                <p className="text-[10px] font-bold text-snp-navy-500 uppercase tracking-widest shrink-0">Choose a collection</p>
          ...
          <div className={`${collectionsVisible ? 'border-t border-[#f0f4f8] pt-4' : 'pt-2'}`}>
            ...
            <div className="grid grid-cols-2 gap-2.5">
              <button
                ...
                className="group flex items-center gap-3 p-3.5 rounded-[14px] bg-[#f8fafc] border border-snp-navy-200 hover:border-snp-indigo-600 hover:bg-snp-indigo-50 transition-all text-left"
              >
                <span className="text-lg shrink-0">🎁</span>
                <div className="flex-1 min-w-0">
                  <p className="text-[13px] font-semibold text-snp-navy-950 leading-tight">Browse All Gifts</p>
                  <p className="text-[11px] text-snp-navy-500 mt-0.5 leading-tight">Full gifts catalog</p>
                </div>
                <ArrowRight className="w-3.5 h-3.5 text-[#c0ccd9] group-hover:text-snp-indigo-600 group-hover:translate-x-0.5 transition-all shrink-0" />
              </button>
              <button
                onClick={() => { onClose(); navigate('/'); }}
                className="group flex items-center gap-3 p-3.5 rounded-[14px] bg-[#f8fafc] border border-snp-navy-200 hover:border-snp-purple-700 hover:bg-snp-purple-50 transition-all text-left"
              >
                <span className="text-lg shrink-0">👕</span>
                <div className="flex-1 min-w-0">
                  <p className="text-[13px] font-semibold text-snp-navy-950 leading-tight">Design & Send Swag</p>
                  <p className="text-[11px] text-snp-navy-500 mt-0.5 leading-tight">Branded gear with your logo</p>
                </div>
                <ArrowRight className="w-3.5 h-3.5 text-[#c0ccd9] group-hover:text-snp-purple-700 group-hover:translate-x-0.5 transition-all shrink-0" />
              </button>
            </div>
          </div>
        </div>
